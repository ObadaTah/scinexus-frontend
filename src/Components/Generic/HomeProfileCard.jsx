...
const HomeProfileCard = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (endpoint === "links") {
          response = await fetch(
            `http://localhost:8080/users/${endpoint}/count`,
            {
    ...
    Promise.all([
      ...
      fetchData("posts"),
      fetchData("links"),
    ]).then(([researchPapers, articles, posts, links]) => {
      setCounts({ researchPapers, articles, posts, links });
    });
  }, [user.id, jwtToken]);
  ...
  return (
    ...
    >
      ...
      <List sx={{ width: "100%", padding: 0, mb: 2 }}>
        <StyledListItem onClick={() => navigate("/connections ")}>
          <ListItemDecorator>
        ...
        <ListItem sx={{ pl: 6 }}>
          Links: {counts.links} {console.log("Links: ", counts.links)}
        </ListItem>
        <ListItem sx={{ pl: 6 }}>
