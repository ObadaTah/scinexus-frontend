...
const reactions = [
  {
    label: "Like",
    serverKey: "LIKE",
    node: <Image src={ASSETS.emojis.like} alt="like" width="30" height="30" />,
  },
  {
    label: "Smart",
    serverKey: "SMART",
  ...
  },
  {
    ...
    node: (
      <Image src={ASSETS.emojis.mindBlowing} alt="mindBlowing" width="30" />
    ),
  },
  {
    label: "Taking Notes",
    ...
    node: (
      <Image src={ASSETS.emojis.takingNotes} alt="takingNotes" width="30" />
    ),
  },
];
...
function ReactionButton({ reactionFromType, reactToId }) {
  ...
  const removeInteraction = async (id) => {
    const response = await fetch(`http://localhost:8080/interactions/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${jwtToken}`,
  ...
  const reactioner = (key) => {
    ...
    const newReaction = reactions.find((reaction) => reaction.label === key);
    if (newReaction) {
  ...
  useEffect(() => {
    getInteraction(reactionFromType, reactToId).then((result) => {
      if (result._embedded && result._embedded.interactionList.length > 0) {
        result._embedded.interactionList.map((interaction) => {
          var existingReaction = null;
          if (interaction.interactorUser.email == user.email) {
            existingReaction = reactions.find(
              (reaction) => reaction.serverKey === interaction.type
            );
    ...
    <>
      ...
      <div className="reactionButton" onClick={toggleReactionsHolder}>
        {selectedReaction ? (
          ...
          <WbIncandescentOutlinedIcon style={{ rotate: "180deg" }} />
          // <svg
      ...
      <Typography style={{ marginLeft: 5 }}>Interactions</Typography>
    </>
  );
}
